...
load_dotenv('.env')
MODEL = "qwen-plus"
WORKING_DIR = "./naive_graphrag_cache_qwen_TEST"
...
async def qwen_model_if_cache(
    ...
    openai_async_client = AsyncOpenAI(
        api_key=os.environ.get("OPENAI_API_KEY"), base_url="https://dashscope.aliyuncs.com/compatible-mode/v1", timeout=90
    )
    ...
    rate_limiter = AsyncLimiter(max_rate=180, time_period=60)  
...
def query():
        ...
        for user, tasks in users.items():   
            ...
            for task, questions in tasks.items():
                ...
                for question in questions:
                    answer = rag.query(question, param=QueryParam(mode="naive"))
    ...
    with open(FOLDER + "/" + "naive-answers.json", "w", encoding="utf-8") as answer_file:
